...
function LessonSession({identity}:{identity:string}) {
 ...
 {interaction==='inspect-columns'&&<>{(['front','side'] as Face[]).map(f=><MathGrid key={f} face={f} title={`${FACE_LABELS[f]}에서 본 모양`} rows={baseProjections[f]} readOnly onCell={(_r,c)=>{setPreset(f);setHighlight(LESSON3_MODEL.filter(b=>f==='front'?b.x===c:b.z===LESSON3_GRID.gridDepth-1-c));updateActivity({inspected:[...new Set([...activity.inspected,f])]});}}/>)}<p>누른 열의 블록이 파랑으로 강조됩니다.</p></>}
 {interaction==='match-views'&&<><div className="spatial-grids">{(['side','top','front'] as Face[]).map((f,i)=><section key={f}><MathGrid face={f} title={['가','나','다'][i]} rows={baseProjections[f]} readOnly floor={f==='top'}/><fieldset><legend>{['가','나','다'][i]}의 방향</legend>{FACES.map(v=><button className="btn" key={v} aria-pressed={activity.mapping[['가','나','다'][i]]===v} onClick={()=>updateActivity({mapping:{...activity.mapping,[['가','나','다'][i]]:v},compared:false})}>{FACE_LABELS[v]}</button>)}</fieldset></section>)}</div>
 ...
 <div className="spatial-grids">{FACES.map(f=><MathGrid key={f} face={f} title={`${FACE_LABELS[f]} · 바뀐 칸 표시`} rows={changedProjections[f]} marked={changedProjections[f].map((row,r)=>row.map((v,c)=>v!==baseProjections[f][r][c]))} floor={f==='top'} readOnly/>)}</div>
...
function AnswerComparison({problem,submitted}:{problem:Problem;submitted:AnswerState|null}) {
 ...
 return <section className="spatial-comparison"><h3>정답과 내 답 비교</h3><p>{problem.revealPolicy.reason}</p><div className="comparison-pair"><section><h4>정답</h4><AnswerRenderer problem={problem} answer={correct} onChange={()=>undefined} readOnly/></section>{submitted&&<section><h4>제출한 내 답</h4><AnswerRenderer problem={problem} answer={submitted} onChange={()=>undefined} readOnly/></section>}</div><p>{problem.answerInput.kind==='block-builder'?'작업판에서 직접 고쳐 쌓아 완성하세요.':'위의 답안 입력에서 직접 고쳐 다시 확인하세요.'}</p></section>;
}
