...
export default function AnswerRenderer({problem,answer,onChange,readOnly=false}:{problem:Problem;answer:AnswerState;onChange:(a:AnswerState)=>void;readOnly?:boolean}) {
 ...
 case 'block-builder':return <ActivityBuilder blocks={answer.blocks} grid={problem.grid} disabled={readOnly} cameraPolicy={problem.cameraPolicy} onChange={blocks=>onChange({kind:'block-builder',blocks})}/>;
