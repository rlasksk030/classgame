...
export default function Representations({given}:{given:ProblemGiven}) {
 ...
 {(['top','front','side'] as const).map(face=>given.projections?.[face]&&<ProjectionGrid key={face} title={{top:'위',front:'앞',side:'옆(오른쪽에서 본 모양)'}[face]} rows={given.projections[face]!} reverseRows={face!=='top'} orientation={face==='top'?'floor':undefined} editable={false} onChange={()=>undefined} valueType="boolean"/>)}
