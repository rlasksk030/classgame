...
export default function ActivityBuilder({blocks,onChange,disabled=false,answer,grid=ACTIVITY_GRID}:{blocks:BlockCoord[];onChange:(blocks:BlockCoord[])=>void;disabled?:boolean;answer?:BlockCoord[];grid?:{gridWidth:number;gridDepth:number;maxHeight:number}}) {
 ...
 <BlockWorld grid={grid} blocks={blocks} selected={selected} layerMax={null} preset={preset} onPreset={setPreset} onBlocksChange={change} onSelect={setSelected} onMessage={setMessage} disabled={disabled} answerGhost={answer}/>
