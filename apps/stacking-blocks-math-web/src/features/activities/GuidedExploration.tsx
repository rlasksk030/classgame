...
import { CONSTRAINT_EXAMPLES, CONSTRAINT_GRID, CONSTRAINT_TARGET, EXPLORATION_GRID, HEIGHT_EDIT_GRID, INFORMATION_MODELS, START_BLOCKS, editExplorationLayer, explorationDisplayProjections, matchesExplorationConditions, type ExplorationKind } from './explorationActivities';
...
export default function GuidedExploration({kind}:{kind:ExplorationKind}) {
 ...
 const heights=toHeightMap(shown,grid), layers=toLayers(shown,grid), projections=explorationDisplayProjections(project(shown,{...grid,maxHeight:Math.max(1,usedHeight(shown))}));
