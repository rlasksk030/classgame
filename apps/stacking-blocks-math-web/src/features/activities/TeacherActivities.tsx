...
export default function TeacherActivities({classId,students}:{classId:string;students:TeacherStudentRow[]}){
 ...
 {selected&&<article className="stack"><h3>{selected.building_name}</h3><p>{selected.reason}</p><p>{selected.description}</p><ActivityBuilder grid={selected.blocks.some(block=>block.x>=ARCHITECTURE_GRID.gridWidth||block.z>=ARCHITECTURE_GRID.gridDepth)?ACTIVITY_GRID:ARCHITECTURE_GRID} blocks={selected.blocks} disabled onChange={()=>undefined}/><Representations given={{projections:project(selected.blocks,selected.blocks.some(block=>block.x>=ARCHITECTURE_GRID.gridWidth||block.z>=ARCHITECTURE_GRID.gridDepth)?ACTIVITY_GRID:ARCHITECTURE_GRID),layers:toLayers(selected.blocks,selected.blocks.some(block=>block.x>=ARCHITECTURE_GRID.gridWidth||block.z>=ARCHITECTURE_GRID.gridDepth)?ACTIVITY_GRID:ARCHITECTURE_GRID)}}/>{selected.layer_notes.map((note,i)=><p key={i}>{i+1}층: {note}</p>)}</article>}
 </section>;
}
