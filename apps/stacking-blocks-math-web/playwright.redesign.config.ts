...
process.env.SPATIAL_QA_OUTPUT=run;
export default defineConfig({testDir:'./e2e',testMatch:['redesign.spec.ts','redesign-phase2.spec.ts','redesign-phase3.spec.ts','redesign-phase4.spec.ts'],fullyParallel:false,workers:1,timeout:90000,retries:0,outputDir:`${run}/evidence`,reporter:[['list'],['json',{outputFile:`${run}/report.json`}],['html',{outputFolder:`${run}/html`,open:'never'}]],metadata:{head,category:'UI_WITH_TEST_DATA',persistence:'LOCAL_ONLY'},
