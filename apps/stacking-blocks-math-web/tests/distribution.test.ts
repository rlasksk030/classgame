...
test("배포판 버전 계약은 package/migration 버전을 노출한다", () => {
  ...
  assert.equal(SCHEMA_VERSION, "202609110012");
});
...
test("앱과 설치된 DB 버전의 호환 상태를 계산한다", () => {
  assert.equal(getVersionState("202609110011").updateRequired, true);
  assert.equal(getVersionState("202609110012").updateRequired, false);
