...
async function flow(page:Page,id:Phase3Lesson){
 ...
 for(const [i,a]of activities.entries()){
  ...
  if(id!==5&&a.problem.answerInput.kind==='block-builder'){const host=page.getByRole('group',{name:'내 답 입력',exact:true});await expect(host.getByRole('button',{name:'위에서 보기',exact:true})).toBeEnabled();await expect(host.getByRole('button',{name:'옆에서 보기',exact:true})).toBeEnabled();}
  if(id===5&&i===0){const canvas=page.getByRole('group',{name:'내 답 입력',exact:true}).locator('canvas');await canvas.scrollIntoViewIfNeeded();await canvas.focus();await page.waitForTimeout(500);await expect(page.getByText('지금은 지정된 시점으로만 살펴볼 수 있어요.',{exact:true})).toHaveCount(2);const before=await canvas.screenshot(),box=(await canvas.boundingBox())!;await page.mouse.move(box.x+15,box.y+15);await page.mouse.down();await page.mouse.move(box.x+65,box.y+30,{steps:10});await page.mouse.up();await page.waitForTimeout(100);const after=await canvas.screenshot();await test.info().attach('fixed-camera-before',{body:before,contentType:'image/png'});await test.info().attach('fixed-camera-after',{body:after,contentType:'image/png'});expect(after.equals(before)).toBe(true);await shot(page,'l5-fixed-oblique');}
