...
type Task = Pick<SeedProblem,'prompt'|'problemType'|'grid'|'given'|'givenBlocks'|'choices'>;
...
export function taskFingerprint(p: Task): string {
  ...
  return JSON.stringify({prompt:p.prompt.normalize('NFKC').replace(/\s+/g,' ').trim(), type:p.problemType, grid:p.grid,
