  ...
  for(let i=0;i<count;i++){
    ...
    if(lesson===1){
      ...
      if(mode===4){
        ...
        item.prompt=mode<2?`빨간 블록의 ${position}에 있는 블록을 골라 보세요.`:mode===5?'쌓기나무는 모두 몇 개인가요? 자리별로 세어 한 번, 층별로 나누어 한 번 세어 보고, 두 방법의 답이 같은지도 확인해 보세요.':mode===2?'2층에 있는 쌓기나무는 몇 개인가요?':'전체 쌓기나무는 몇 개인가요?';
      }
    }
    ...
    else if(lesson===12){const mode=i%8; if(mode===0)item=base(lesson,i,blocks,'CAMERA_DIRECTION',{projections:{front:p.front},shownFrom:'front',allowRotate:true},{kind:'direction',value:'front'},grid,'exact',selectedTemplate); else if(mode===1)item=base(lesson,i,blocks,'PROJECTION_DRAW',{projections:p,allowRotate:true},{kind:'projections',projections:p},grid,'exact',selectedTemplate); else if(mode===3)item=base(lesson,i,blocks,'HEIGHTMAP_FROM_BUILD',{allowRotate:true},{kind:'heightMap',heightMap:h},grid,'exact',selectedTemplate); else if(mode===5)item=base(lesson,i,blocks,'LAYER_DRAW',{layers:cells(blocks,grid),allowRotate:true},{kind:'layers',layers:cells(blocks,grid)},grid,'exact',selectedTemplate); else if(mode===6)item=base(lesson,i,blocks,'COUNT_AMBIGUOUS',{/* allowRotate:true 최소 수정: 막혀 있고 위/앞/옆·숫자 지도·층별 자료가 전혀 없으면 학생이 실제 총 개수를 추론할 방법이 없었다. oracle/reports/cross-check-report.md 참고. */allowRotate:true},{kind:'count',value:blocks.length},grid,'exact',selectedTemplate); else if(mode===7)item=base(lesson,i,blocks,'CHOICE',{allowRotate:true},{kind:'choice',index:0},grid,'exact',selectedTemplate); else item=base(lesson,i,blocks,'COUNT',{allowRotate:true},{kind:'count',value:blocks.length},grid,'exact',selectedTemplate);
      // modes 2/4 (COUNT, via the final else above) and mode 6 (COUNT_AMBIGUOUS) never
      // set an explicit prompt, so they fell through to base()'s generic non-numeric
      // default ("...문제를 해결해 보세요.") while still expecting a numeric answer --
      // the same prompt/answer-type mismatch reported for lesson 1 mode 5.
      if(mode===2||mode===4||mode===6)item.prompt='쌓기나무를 자유롭게 돌려 보고, 전체 개수는 몇 개인지 구해 보세요.';
    }
